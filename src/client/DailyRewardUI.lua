--[[
    ...
    ────────────────────────────────────────────────
    v2.0 "BIGGER" (2026-09-25). The daily reward calendar.

        DAILY REWARD                                         (X)
        Come back every day for more cash!
        [DAY 1] [DAY 2] [DAY 3] [DAY 4] [DAY 5] [DAY 6] [DAY 7]
         $250    $400    $600    $800  $1,000  $1,500  $3,000 + GOLD BAG
        [        DAY 3 — $600 — CLAIM!        ]

    • On join, if a reward is ready, the card pops up by itself.
    • A small "DAILY" button on the left edge reopens it any time (a gold dot
      shows when a reward is waiting).
    • Days already claimed get a check, today's box glows gold.
...
function DailyRewardUI:_build()
    ...
    -- ── the small reopen button (left edge, under Roblox's chat) ──
    local open = Instance.new("TextButton")
    open.Name = "DailyButton"
    open.Text = ""
    open.AutoButtonColor = false
    open.AnchorPoint = Vector2.new(0, 0.5)
    open.Position = UDim2.new(0, 16, 0.3, 0)
    open.Size = UDim2.fromOffset(92, 36)
    open.BackgroundColor3 = T.bg
    open.BackgroundTransparency = 0.18
    open.BorderSizePixel = 0
    open.Parent = screen
    UITheme.corner(open, 18)
    UITheme.stroke(open, T.gold, 0.55)
    UITheme.label({ Text = "DAILY", Size = UDim2.fromScale(1, 1), TextXAlignment = Enum.TextXAlignment.Center,
        FontFace = UITheme.F.display, TextSize = 15, TextColor3 = T.gold, Parent = open })
    local dot = frame({ Name = "Ready", AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(1, -4, 0, 4),
        Size = UDim2.fromOffset(12, 12), BackgroundColor3 = T.danger, BackgroundTransparency = 0, Visible = false })
    UITheme.corner(dot, 6)
    UITheme.stroke(dot, T.bg, 0, 2)
    dot.Parent = open
    local openScale = Instance.new("UIScale")
    openScale.Parent = open
    open.MouseEnter:Connect(function() tween(openScale, 0.12, { Scale = 1.05 }) end)
    ...
    group.Size = UDim2.fromOffset(W + 4, H + 4)
    ...
    UITheme.caption("Daily reward", { Position = UDim2.fromOffset(24, 18), Size = UDim2.fromOffset(300, 14),
        TextSize = 12, TextColor3 = T.gold }).Parent = panel
    UITheme.label({ Text = "Come back every day!", Position = UDim2.fromOffset(24, 32), Size = UDim2.fromOffset(460, 32),
        FontFace = UITheme.F.display, TextSize = 28 }).Parent = panel
    local sub = UITheme.label({ Text = "Every day in a row pays more. Day 7 = the big one!",
        Position = UDim2.fromOffset(24, 64), Size = UDim2.new(1, -48, 0, 18),
        FontFace = UITheme.F.medium, TextSize = 14, TextColor3 = T.muted })
    sub.Parent = panel
    ...
    for _, rot in ipairs({ 45, -45 }) do
        local bar = frame({ AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.5, 0.5),
            Size = UDim2.fromOffset(2, 14), Rotation = rot, BackgroundColor3 = T.text, BackgroundTransparency = 0 })
    ...
    -- the 7 day boxes
    local row = frame({ Name = "Days", Position = UDim2.fromOffset(24, 96), Size = UDim2.new(1, -48, 0, 128) })
    row.Parent = panel
    ...
    layout.Parent = row
    local boxW = math.floor((W - 48 - 8 * 6) / 7)
    self._boxes = {}
    for i = 1, 7 do
        local box = frame({ LayoutOrder = i, Size = UDim2.fromOffset(boxW, 128), BackgroundColor3 = T.bgRaised,
            BackgroundTransparency = 0.3 })
        UITheme.corner(box, 12)
        local stroke = UITheme.stroke(box, T.line, 0.88)
        box.Parent = row
        local dayL = UITheme.label({ Position = UDim2.fromOffset(0, 10), Size = UDim2.new(1, 0, 0, 14),
            Text = "DAY " .. i, TextXAlignment = Enum.TextXAlignment.Center, FontFace = UITheme.F.bold,
            TextSize = 12, TextColor3 = T.muted })
        dayL.Parent = box
        -- a stack of cash (bigger stack each day)
        local coin = frame({ AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0, 52),
            Size = UDim2.fromOffset(i == 7 and 40 or 30, i == 7 and 40 or 30), BackgroundColor3 = i == 7 and T.gold or T.money,
            BackgroundTransparency = 0 })
        UITheme.corner(coin, i == 7 and 12 or 15)
        coin.Parent = box
        local sign = UITheme.label({ Size = UDim2.fromScale(1, 1), Text = "$", TextXAlignment = Enum.TextXAlignment.Center,
            FontFace = UITheme.F.display, TextSize = i == 7 and 24 or 18, TextColor3 = T.bg })
        sign.Parent = coin
        local amt = UITheme.label({ Position = UDim2.fromOffset(0, 78), Size = UDim2.new(1, 0, 0, 20),
        ...
        amt.Parent = box
        local extra = UITheme.label({ Position = UDim2.fromOffset(0, 98), Size = UDim2.new(1, 0, 0, 14),
            Text = i == 7 and "+ GOLD BAG" or "", TextXAlignment = Enum.TextXAlignment.Center,
            FontFace = UITheme.F.bold, TextSize = 11, TextColor3 = T.gold })
        extra.Parent = box
        -- claimed check
        local check = frame({ Name = "Check", AnchorPoint = Vector2.new(1, 0), Position = UDim2.new(1, -6, 0, 6),
        ...
        local short = frame({ AnchorPoint = Vector2.new(0.5, 0.5), Size = UDim2.fromOffset(2, 5),
            Position = UDim2.fromOffset(6.6, 10.4), Rotation = -45, BackgroundColor3 = T.bg, BackgroundTransparency = 0 })
        short.Parent = check
        local long = frame({ AnchorPoint = Vector2.new(0.5, 0.5), Size = UDim2.fromOffset(2, 9),
            Position = UDim2.fromOffset(10.4, 8.6), Rotation = 39, BackgroundColor3 = T.bg, BackgroundTransparency = 0 })
        long.Parent = check
...
function DailyRewardUI:_render()
    ...
    for i, b in ipairs(self._boxes) do
        ...
        if today then
            ...
            b.box.BackgroundTransparency = claimed and 0.55 or 0.3
...
function DailyRewardUI:_claim()
    ...
    task.spawn(function()
        ...
        self._busy = false
        if not res then
            self._sub.Text = "Couldn't reach the server. Try again!"
            self._sub.TextColor3 = T.danger
            self:_render()
            return
        end
        if res.ok then
            self._justClaimed = true
            self._sub.Text = res.msg or "Reward claimed!"
            self._sub.TextColor3 = T.money
            ...
            self._sub.Text = res.msg or "Not yet!"
            self._sub.TextColor3 = T.danger
            self:_setStatus(res)
        end
    end)
...
function DailyRewardUI:_layout()
    ...
    self._fitScale.Scale = math.clamp(math.min((vp.X - 24) / (W + 4), (vp.Y - 24) / (H + 4)), 0.5, 1)
end
...
function DailyRewardUI:open()
    ...
    self._sub.Text = "Every day in a row pays more. Day 7 = the big one!"
...
function DailyRewardUI:start()
    ...
    task.spawn(function()
        ...
        -- give the save a moment to load, then pop the card if a reward is waiting
        for _ = 1, 10 do
        ...
        local st = self._status
        if st and st.ok and st.claimable then
            task.wait(4)   -- let the join title card play first
            self:open()
        end
    end)
